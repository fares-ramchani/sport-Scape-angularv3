...
export class Composant1partenaireComponent implements OnInit {
  ...
  constructor(private formBuilder: FormBuilder,private ServiceStadeService:ServiceStadeService, private toastr:ToastrService,) {}
  ...
  ajouterStade(){
    ...
    this.ServiceStadeService.ajouterStade(uploadData).subscribe({
      next: data => {
        this.toastr.success(" Votre creation a été effectué avec succès")
      },
      error: error => {
