...
export class ServiceLoginService {
    ...
    return this.http.get<Token>("http://localhost:8081/auth/callback?code=" + code, {observe: "response"})
      .pipe(map((response: HttpResponse<Token>) => {
        if (response.status === 200 && response.body !== null) {
          ...
          localStorage.setItem("isAuthenticated",newurlimage)
  ...
  getTokenAcees(code: string): Observable<boolean> {
    return this.http.get<Token>("http://localhost:8081/auth/AcessToken?code=" + code, {observe: "response"})
      .pipe(map((response: HttpResponse<Token>) => {
        if (response.status === 200 && response.body !== null) {
